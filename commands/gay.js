...
const CATEGORY = "gay"; // Define category for easy reuse
...
const createCooldownEmbed = (timeLeft) => { // Renamed and uses EmbedBuilder
    return new EmbedBuilder()
        ...
        .setTitle('⏰ Patience, Pride Seeker!')
        .setDescription(`You must wait **${timeLeft.toFixed(1)}s** before celebrating more love. Good things come to those who wait for pride!`)
        .setTimestamp();
};
...
const generateImageEmbedAndComponents = async () => {
    ...
    const embed = new EmbedBuilder()
        .setColor(0xE40303) // Red from rainbow flag
        .setTitle('Celebrating Love and Pride! 🏳️‍🌈')
        .setImage(imageUrl)
        .setTimestamp()
        .setFooter({ text: `Category: ${CATEGORY}` });

    const reloadButton = new ButtonBuilder()
        .setCustomId(`${CATEGORY}_button_reload`) // e.g., gay_button_reload
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName(CATEGORY)
        .setDescription('Delivers images celebrating gay love. 🏳️‍🌈'),
    ...
    async handleComponent(interaction, componentArgs) {
        // componentArgs: ['button', 'reload'] from customId like 'gay_button_reload'
        if (!interaction.channel || !interaction.channel.nsfw) {
            return interaction.reply({ embeds: [createNsfwOnlyEmbed()], ephemeral: true });
        }

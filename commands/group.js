...
const imagePreloader = new ImagePreloader("group");
const cooldowns = new Map(); // UserID -> timestamp

// Main function to generate the image payload
const generateGroupPayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0x4682B4) // Steel Blue for group
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### The More, The Merrier! 🎉🧑‍🤝‍🧑')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('A lively group scene.')
                )
            );

        const reloadButton = new ButtonBuilder()
            .setCustomId('group_button_reload')
            .setLabel('🧑‍🤝‍🧑 More Group Fun!')
        ...
        const errorPayload = createApiErrorPayload("group");
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('group')
        .setDescription('Delivers images of group activities. 🎉🧑‍🤝‍🧑'),
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generateGroupPayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generateGroupPayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {await interaction.deferUpdate();
                const newPayload = await generateGroupPayload(interaction);
                // Check if generateGroupPayload returned an error payload
            ...
            }  catch (error) {console.error('Error handling group reload button:', error);
                // General error handling for other issues, send ephemeral error
                const errorPayload = createApiErrorPayload("group"); // Use the updated error payload function
                 if (interaction.replied || interaction.deferred) {

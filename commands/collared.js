...
const imagePreloader = new ImagePreloader("collared");

// Shared function to generate the response payload using Components V2
const generateCollaredPayload = async (cachedUrl = null) => {
    try {
        ...
        if (!cachedUrl) {
            ...
            cache.lastCollaredUrl = imageUrl;
            saveCache(cache);
        }
        ...
        const container = new ContainerBuilder()
            ...
            .addTextDisplayComponents( // Add a title
                textDisplay => textDisplay
                    .setContent('### Behold! A mark of devotion.') // Markdown for a nice heading
            )
                ...
                mediaGallery => mediaGallery
                    .addItems( // Add the image/media from the fetched URL
                        mediaGalleryItem => mediaGalleryItem
                            ...
                            .setURL(imageUrl)
                            .setDescription('Bound by choice.') // Alt text.
                    )
            );

        // Add the Reload button.
        const reloadButton = new ButtonBuilder()
            .setCustomId('collared_button_reload') // Unique ID for this command's reload button.
            .setLabel('🔃 Reload')
        ...
        console.error('Error fetching collared image:', error);
        ...
        const errorContainer = new ContainerBuilder()
            ...
            .addTextDisplayComponents(
                textDisplay => textDisplay
                    .setContent('Failed to fetch the requested image. The collar seems to have slipped away.')
            );
...
const updateButtonCountdown = async (interaction, seconds, cachedUrl) => {
    const container = new ContainerBuilder()
        ...
        .addTextDisplayComponents(
            textDisplay => textDisplay
                .setContent('### Behold! A mark of devotion.')
        )
        .addSeparatorComponents(
            separator => separator
        ...
        .addMediaGalleryComponents(
            mediaGallery => mediaGallery
                .addItems(
                    mediaGalleryItem => mediaGalleryItem
                        .setURL(cachedUrl)
                        .setDescription('Bound by choice.')
                )
        );

    const reloadButton = new ButtonBuilder()
        .setCustomId('collared_button_reload')
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('collared')
        .setDescription('Displays symbols of submission.'), // A descriptive explanation.

    // Slash Command Execution
    async slashExecute(interaction) {
        const cooldownCheck = checkCooldown(interaction.user.id, 'collared');
        ...
        const payload = await generateCollaredPayload();
        
        // Set cooldown after successful execution
        setCooldown(interaction.user.id, 'collared');
    ...
    async prefixExecute(message, args) {
        const cooldownCheck = checkCooldown(message.author.id, 'collared');
        ...
        // Set cooldown after successful execution
        setCooldown(message.author.id, 'collared');
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {
                ...
                const cache = loadCache();
                const cachedUrl = cache.lastCollaredUrl;
                ...
                const newPayload = await generateCollaredPayload();

                // Edit the original message.
                await interaction.editReply(newPayload);

            } catch (error) {
                console.error('Error handling collared reload button:', error);

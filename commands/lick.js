...
const CATEGORY = "lick"; // Define category for easy reuse
...
const createCooldownEmbed = (timeLeft) => { // Renamed and uses EmbedBuilder
    return new EmbedBuilder()
        ...
        .setTitle('⏰ Easy There, Licker!')
        .setDescription(`You must wait **${timeLeft.toFixed(1)}s** before another taste. Savor the moment!`)
        .setTimestamp();
};
...
const generateImageEmbedAndComponents = async () => {
    ...
    const embed = new EmbedBuilder()
        .setColor(0xFFC0CB) // Pink for lick
        .setTitle('A Tasty Lick! 👅💦')
        .setImage(imageUrl)
        .setTimestamp()
        .setFooter({ text: `Category: ${CATEGORY}` });

    const reloadButton = new ButtonBuilder()
        .setCustomId(`${CATEGORY}_button_reload`) // e.g., lick_button_reload
        .setLabel('👅 More Licks!')
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName(CATEGORY)
        .setDescription('Delivers images of skillful licks. 👅💦'),
    ...
    async handleComponent(interaction, componentArgs) {
        // componentArgs: ['button', 'reload'] from customId like 'lick_button_reload'
        if (!interaction.channel || !interaction.channel.nsfw) {
            return interaction.reply({ embeds: [createNsfwOnlyEmbed()], ephemeral: true });
        }

...
const CATEGORY = "blowjub"; // Define category for easy reuse, matches API term
...
const createCooldownEmbed = (timeLeft) => { // Renamed and uses EmbedBuilder
    return new EmbedBuilder()
        ...
        .setTitle('⏰ Patience, Enthusiast') // Adjusted title for context
        .setDescription(`You must wait **${timeLeft.toFixed(1)}s** before requesting another skillful performance. Quality takes time.`) // Adjusted description
        .setTimestamp();
};
...
const generateImageEmbedAndComponents = async () => {
    ...
    const embed = new EmbedBuilder()
        .setColor(0xFFC0CB) // Pink for blowjub
        .setTitle('A Skillful Performance! 👄🍆')
        .setImage(imageUrl)
        .setTimestamp()
        .setFooter({ text: `Category: ${CATEGORY}` });

    const reloadButton = new ButtonBuilder()
        .setCustomId(`${CATEGORY}_button_reload`) // e.g., blowjub_button_reload
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName(CATEGORY) // Using API's term
    ...
    async handleComponent(interaction, componentArgs) {
        // componentArgs: ['button', 'reload'] from customId like 'blowjub_button_reload'
        if (!interaction.channel || !interaction.channel.nsfw) {
            return interaction.reply({ embeds: [createNsfwOnlyEmbed()], ephemeral: true });
        }

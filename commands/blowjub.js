...
const imagePreloader = new ImagePreloader("blowjub"); // API uses "blowjub"
const cooldowns = new Map(); // UserID -> timestamp

// Main function to generate the image payload
const generateBlowjubPayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0xFFC0CB) // Pink for blowjub
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### A Skillful Performance! 👄🍆')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('A delightful oral presentation.')
                )
            );

        const reloadButton = new ButtonBuilder()
            .setCustomId('blowjub_button_reload')
            .setLabel('👄 More Action!')
            .setStyle(ButtonStyle.Danger); // Danger for intense action
        ...
        const errorPayload = createApiErrorPayload("blowjub");
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('blowjub') // Using API's term
        .setDescription('Delivers skillful blowjob images. 👄🍆'),
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generateBlowjubPayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generateBlowjubPayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {await interaction.deferUpdate();
                const newPayload = await generateBlowjubPayload(interaction);
                // Check if generateBlowjubPayload returned an error payload
            ...
            }  catch (error) {console.error('Error handling blowjub reload button:', error);
                // General error handling for other issues, send ephemeral error
                const errorPayload = createApiErrorPayload("blowjub"); // Use the updated error payload function
                 if (interaction.replied || interaction.deferred) {

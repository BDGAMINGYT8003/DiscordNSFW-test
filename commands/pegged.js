...
const imagePreloader = new ImagePreloader("pegged");
const cooldowns = new Map(); // UserID -> timestamp

// Main function to generate the image payload
const generatePeggedPayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0x8A2BE2) // BlueViolet for pegged
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### Taking Charge! ♀️♂️🔩')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('A powerful pegging scene.')
                )
            );

        const reloadButton = new ButtonBuilder()
            .setCustomId('pegged_button_reload')
            .setLabel('🔩 More Pegging!')
        ...
        console.error('Error fetching pegged image:', error);
        const errorPayload = createApiErrorPayload("pegged");
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('pegged')
        .setDescription('Delivers images of powerful pegging. ♀️♂️🔩'),
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generatePeggedPayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generatePeggedPayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {await interaction.deferUpdate();
                const newPayload = await generatePeggedPayload(interaction);
                // Check if generatePeggedPayload returned an error payload
            ...
            }  catch (error) {console.error('Error handling pegged reload button:', error);
                // General error handling for other issues, send ephemeral error
                const errorPayload = createApiErrorPayload("pegged"); // Use the updated error payload function
                 if (interaction.replied || interaction.deferred) {

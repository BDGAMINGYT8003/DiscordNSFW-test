...
const imagePreloader = new ImagePreloader("hentai");
const cooldowns = new Map(); // UserID -> timestamp

// Main function to generate the image payload
const generateHentaiPayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0xFF69B4) // Hot Pink, often associated with anime/hentai themes
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### Animated Ecstasy! 🎨')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('A fine piece of hentai art.')
                )
            );

        const reloadButton = new ButtonBuilder()
            .setCustomId('hentai_button_reload')
            .setLabel('🎨 More Hentai!')
        ...
        console.error('Error fetching hentai image:', error);
        const errorPayload = createApiErrorPayload("hentai");
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('hentai')
        .setDescription('Delivers artistic hentai images. 🎨'),
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generateHentaiPayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generateHentaiPayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {await interaction.deferUpdate();
                const newPayload = await generateHentaiPayload(interaction);
                // Check if generateHentaiPayload returned an error payload
                ...
                }catch (error) {console.error('Error handling hentai reload button:', error);
                // General error handling for other issues, send ephemeral error
                const errorPayload = createApiErrorPayload("hentai"); // Use the updated error payload function
                 if (interaction.replied || interaction.deferred) {

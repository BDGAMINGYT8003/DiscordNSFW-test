...
const CATEGORY = "pussy"; // Define category for easy reuse
...
const createCooldownEmbed = (timeLeft) => { // Renamed and uses EmbedBuilder
    return new EmbedBuilder()
        ...
        .setTitle('⏰ Patience, Pussy Appreciator!')
        .setDescription(`You must wait **${timeLeft.toFixed(1)}s** before admiring another view. Good things come to those who wait!`)
        .setTimestamp();
};
        ...
        setImmediate(async () => {
            ...
            try {
                while (this.cache.length < this.targetCacheSize) {
                    ...
                    // await new Promise(resolve => setTimeout(resolve, 200)); // Small delay - removed based on other files
                }
            } catch (error) {
...
const generateImageEmbedAndComponents = async () => {
    ...
    const embed = new EmbedBuilder()
        .setColor(0xFF007F) // Pussy pink
        .setTitle('Behold! A Gift of Pussy! 😽')
        .setImage(imageUrl)
        .setTimestamp()
        .setFooter({ text: `Category: ${CATEGORY}` });

    const reloadButton = new ButtonBuilder()
        .setCustomId(`${CATEGORY}_button_reload`) // e.g., pussy_button_reload
        .setLabel('🔃 More Pussy!')
...
module.exports = {
    ...
    async handleComponent(interaction, componentArgs) {
        // componentArgs: ['button', 'reload'] from customId like 'pussy_button_reload'
        if (!interaction.channel || !interaction.channel.nsfw) {
            return interaction.reply({ embeds: [createNsfwOnlyEmbed()], ephemeral: true });
        }

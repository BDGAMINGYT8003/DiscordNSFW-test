...
const imagePreloader = new ImagePreloader("futa");
const cooldowns = new Map(); // UserID -> timestamp

// Main function to generate the image payload
const generateFutaPayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0xDA70D6) // Orchid for futa
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### The Best of Both Worlds! ♀️♂️')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('A captivating futanari view.')
                )
            );

        const reloadButton = new ButtonBuilder()
            .setCustomId('futa_button_reload')
            .setLabel('♀️♂️ More Futa!')
        ...
        console.error('Error fetching futa image:', error);
        const errorPayload = createApiErrorPayload("futa");
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('futa')
        .setDescription('Delivers captivating futanari images. ♀️♂️'),
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generateFutaPayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generateFutaPayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {await interaction.deferUpdate();
                const newPayload = await generateFutaPayload(interaction);
                // Check if generateFutaPayload returned an error payload
                ...
                }catch (error) {console.error('Error handling futa reload button:', error);
                // General error handling for other issues, send ephemeral error
                const errorPayload = createApiErrorPayload("futa"); // Use the updated error payload function
                 if (interaction.replied || interaction.deferred) {

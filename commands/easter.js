...
const generateEasterPayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0xFFD700) // Gold for Easter
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### Happy Easter! A Special Treat! 🐰🥚')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('A festive Easter surprise.')
                )
            );

        const reloadButton = new ButtonBuilder()
            .setCustomId('easter_button_reload')
            .setLabel('🥚 More Easter Fun!')
        ...
        const errorPayload = createApiErrorPayload("easter");
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('easter')
        .setDescription('Delivers a festive Easter image. 🐰🥚'),
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generateEasterPayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generateEasterPayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {await interaction.deferUpdate();
                const newPayload = await generateEasterPayload(interaction);
                // Check if generateEasterPayload returned an error payload
            ...
            }  catch (error) {console.error('Error handling easter reload button:', error);
                // General error handling for other issues, send ephemeral error
                const errorPayload = createApiErrorPayload("easter"); // Use the updated error payload function
                 if (interaction.replied || interaction.deferred) {

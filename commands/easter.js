...
const imagePreloader = new ImagePreloader("easter");

// Shared function to generate the response payload using Components V2
const generateEasterPayload = async (cachedUrl = null) => {
    try {
        ...
        const container = new ContainerBuilder()
            ...
            .addTextDisplayComponents( // Add a title
                textDisplay => textDisplay
                    .setContent('### Behold! A seasonal celebration.') // Markdown for a nice heading
            )
                ...
                mediaGallery => mediaGallery
                    .addItems( // Add the image/media from the fetched URL
                        mediaGalleryItem => mediaGalleryItem
                            ...
                            .setURL(imageUrl)
                            .setDescription('A festive treat.') // Alt text.
                    )
            );

        // Add the Reload button.
        const reloadButton = new ButtonBuilder()
            .setCustomId('easter_button_reload') // Unique ID for this command's reload button.
            .setLabel('🔃 Reload')
        ...
        console.error('Error fetching easter image:', error);
        ...
        const errorContainer = new ContainerBuilder()
            ...
            .addTextDisplayComponents(
                textDisplay => textDisplay
                    .setContent('Failed to fetch the requested image. The bunny seems to have hopped away.')
            );
...
const updateButtonCountdown = async (interaction, seconds, cachedUrl) => {
    const container = new ContainerBuilder()
        ...
        .addTextDisplayComponents(
            textDisplay => textDisplay
                .setContent('### Behold! A seasonal celebration.')
        )
        .addSeparatorComponents(
            separator => separator
        ...
        .addMediaGalleryComponents(
            mediaGallery => mediaGallery
                .addItems(
                    mediaGalleryItem => mediaGalleryItem
                        .setURL(cachedUrl)
                        .setDescription('A festive treat.')
                )
        );

    const reloadButton = new ButtonBuilder()
        .setCustomId('easter_button_reload')
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('easter')
        .setDescription('Presents seasonal delights.'), // A festive description.

    // Slash Command Execution
    async slashExecute(interaction) {
        const cooldownCheck = checkCooldown(interaction.user.id, 'easter');
        ...
        const payload = await generateEasterPayload();
        
        // Set cooldown after successful execution
        setCooldown(interaction.user.id, 'easter');
    ...
    async prefixExecute(message, args) {
        const cooldownCheck = checkCooldown(message.author.id, 'easter');
        ...
        // Set cooldown after successful execution
        setCooldown(message.author.id, 'easter');
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {
                ...
                const cache = loadCache();
                const cachedUrl = cache.lastEasterUrl;
                ...
                const newPayload = await generateEasterPayload();

                // Edit the original message.
                await interaction.editReply(newPayload);

            } catch (error) {
                console.error('Error handling easter reload button:', error);

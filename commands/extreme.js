...
const imagePreloader = new ImagePreloader("extreme");

// Shared function to generate the response payload using Components V2
const generateExtremePayload = async (cachedUrl = null) => {
    try {
        ...
        const container = new ContainerBuilder()
            ...
            .addTextDisplayComponents( // Add a title
                textDisplay => textDisplay
                    .setContent('### Behold! Beyond the ordinary.') // Markdown for a nice heading
            )
                ...
                mediaGallery => mediaGallery
                    .addItems( // Add the image/media from the fetched URL
                        mediaGalleryItem => mediaGalleryItem
                            ...
                            .setURL(imageUrl)
                            .setDescription('Pushing all boundaries.') // Alt text.
                    )
            );

        // Add the Reload button.
        const reloadButton = new ButtonBuilder()
            .setCustomId('extreme_button_reload') // Unique ID for this command's reload button.
            .setLabel('🔃 Reload')
        ...
        console.error('Error fetching extreme image:', error);
        ...
        const errorContainer = new ContainerBuilder()
            ...
            .addTextDisplayComponents(
                textDisplay => textDisplay
                    .setContent('Failed to fetch the requested image. Even the extreme has its limits today.')
            );
...
const updateButtonCountdown = async (interaction, seconds, cachedUrl) => {
    const container = new ContainerBuilder()
        ...
        .addTextDisplayComponents(
            textDisplay => textDisplay
                .setContent('### Behold! Beyond the ordinary.')
        )
        .addSeparatorComponents(
            separator => separator
        ...
        .addMediaGalleryComponents(
            mediaGallery => mediaGallery
                .addItems(
                    mediaGalleryItem => mediaGalleryItem
                        .setURL(cachedUrl)
                        .setDescription('Pushing all boundaries.')
                )
        );

    const reloadButton = new ButtonBuilder()
        .setCustomId('extreme_button_reload')
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('extreme')
        .setDescription('Ventures into the most intense territories.'), // A descriptive explanation.

    // Slash Command Execution
    async slashExecute(interaction) {
        const cooldownCheck = checkCooldown(interaction.user.id, 'extreme');
        ...
        const payload = await generateExtremePayload();
        
        // Set cooldown after successful execution
        setCooldown(interaction.user.id, 'extreme');
    ...
    async prefixExecute(message, args) {
        const cooldownCheck = checkCooldown(message.author.id, 'extreme');
        ...
        // Set cooldown after successful execution
        setCooldown(message.author.id, 'extreme');
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {
                ...
                const cache = loadCache();
                const cachedUrl = cache.lastExtremeUrl;
                ...
                const newPayload = await generateExtremePayload();

                // Edit the original message.
                await interaction.editReply(newPayload);

            } catch (error) {
                console.error('Error handling extreme reload button:', error);

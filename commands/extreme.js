...
const CATEGORY = "extreme"; // Define category for easy reuse
...
const createCooldownEmbed = (timeLeft) => { // Renamed and uses EmbedBuilder
    return new EmbedBuilder()
        ...
        .setTitle('⏰ Brace Yourself! Cooldown Active')
        .setDescription(`You must wait **${timeLeft.toFixed(1)}s** before diving into another extreme scene. Even thrill-seekers need a moment.`)
        .setTimestamp();
};
...
const generateImageEmbedAndComponents = async () => {
    ...
    const embed = new EmbedBuilder()
        .setColor(0xFF4500) // OrangeRed for Extreme
        .setTitle('Pushing the Limits! 🌋')
        .setImage(imageUrl)
        .setTimestamp()
        .setFooter({ text: `Category: ${CATEGORY}` });

    const reloadButton = new ButtonBuilder()
        .setCustomId(`${CATEGORY}_button_reload`) // e.g., extreme_button_reload
...
module.exports = {
    ...
    async handleComponent(interaction, componentArgs) {
        // componentArgs: ['button', 'reload'] from customId like 'extreme_button_reload'
        if (!interaction.channel || !interaction.channel.nsfw) {
            return interaction.reply({ embeds: [createNsfwOnlyEmbed()], ephemeral: true });
        }

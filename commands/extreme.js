...
const generateExtremePayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0xFF4500) // OrangeRed for Extreme
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### Pushing the Limits! 🌋')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('An intense and extreme view.')
                )
            );

        const reloadButton = new ButtonBuilder()
            .setCustomId('extreme_button_reload')
            .setLabel('🌋 More Extreme!')
            .setStyle(ButtonStyle.Danger); // Danger for extreme content
        ...
        const errorPayload = createApiErrorPayload("extreme");
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('extreme')
        .setDescription('Delivers intense and extreme images. 🌋'),
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generateExtremePayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generateExtremePayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {await interaction.deferUpdate();
                const newPayload = await generateExtremePayload(interaction);
                // Check if generateExtremePayload returned an error payload
                ...
                }catch (error) {console.error('Error handling extreme reload button:', error);
                // General error handling for other issues, send ephemeral error
                const errorPayload = createApiErrorPayload("extreme"); // Use the updated error payload function
                 if (interaction.replied || interaction.deferred) {

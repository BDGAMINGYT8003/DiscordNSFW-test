...
const imagePreloader = new ImagePreloader("lesbian");
const cooldowns = new Map(); // UserID -> timestamp

// Main function to generate the image payload
const generateLesbianPayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0xD2691E) // Chocolate/Brown, often associated with lesbian pride flag colors
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### Celebrating Women Loving Women! 👭❤️')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('A beautiful display of lesbian love.')
                )
            );

        const reloadButton = new ButtonBuilder()
            .setCustomId('lesbian_button_reload')
            .setLabel('👭 More Love!')
        ...
        console.error('Error fetching lesbian image:', error);
        const errorPayload = createApiErrorPayload("lesbian");
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('lesbian')
        .setDescription('Delivers images celebrating lesbian love. 👭❤️'),
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generateLesbianPayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generateLesbianPayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {await interaction.deferUpdate();
                const newPayload = await generateLesbianPayload(interaction);
                // Check if generateLesbianPayload returned an error payload
                ...
                }catch (error) {console.error('Error handling lesbian reload button:', error);
                // General error handling for other issues, send ephemeral error
                const errorPayload = createApiErrorPayload("lesbian"); // Use the updated error payload function
                 if (interaction.replied || interaction.deferred) {

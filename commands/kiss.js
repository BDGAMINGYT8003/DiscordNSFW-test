...
const imagePreloader = new ImagePreloader("kiss");
const cooldowns = new Map(); // UserID -> timestamp

// Main function to generate the image payload
const generateKissPayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0xFFC0CB) // Pink for kiss
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### A Tender Kiss! 😘💋')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('A sweet and passionate kiss.')
                )
            );

        const reloadButton = new ButtonBuilder()
            .setCustomId('kiss_button_reload')
            .setLabel('💋 More Kisses!')
        ...
        const errorPayload = createApiErrorPayload("kiss");
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('kiss')
        .setDescription('Delivers images of tender kisses. 😘💋'),
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generateKissPayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generateKissPayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {await interaction.deferUpdate();
                const newPayload = await generateKissPayload(interaction);
                // Check if generateKissPayload returned an error payload
            ...
            }  catch (error) {console.error('Error handling kiss reload button:', error);
                // General error handling for other issues, send ephemeral error
                const errorPayload = createApiErrorPayload("kiss"); // Use the updated error payload function
                 if (interaction.replied || interaction.deferred) {

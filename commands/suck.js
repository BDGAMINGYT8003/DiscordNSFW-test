...
const imagePreloader = new ImagePreloader("suck");
const cooldowns = new Map(); // UserID -> timestamp

// Main function to generate the image payload
const generateSuckPayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0xFF1493) // DeepPink for suck
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### Time for a Good Sucking! 😮')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('A delightful sucking scene.')
                )
            );

        const reloadButton = new ButtonBuilder()
            .setCustomId('suck_button_reload')
            .setLabel('😮 More Sucking Action!')
        ...
        console.error('Error fetching suck image:', error);
        const errorPayload = createApiErrorPayload("suck");
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('suck')
        .setDescription('Delivers images of intense sucking. 😮'),
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generateSuckPayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generateSuckPayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {await interaction.deferUpdate();
                const newPayload = await generateSuckPayload(interaction);
                // Check if generateSuckPayload returned an error payload
                ...
                }catch (error) {console.error('Error handling suck reload button:', error);
                // General error handling for other issues, send ephemeral error
                const errorPayload = createApiErrorPayload("suck"); // Use the updated error payload function
                 if (interaction.replied || interaction.deferred) {

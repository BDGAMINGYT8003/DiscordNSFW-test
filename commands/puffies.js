...
const imagePreloader = new ImagePreloader("puffies");
const cooldowns = new Map(); // UserID -> timestamp

// Main function to generate the image payload
const generatePuffiesPayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0xFFB6C1) // LightPink for puffies
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### Delicate Puffies on Display! 🌸')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('A lovely view of puffy nipples.')
                )
            );

        const reloadButton = new ButtonBuilder()
            .setCustomId('puffies_button_reload')
            .setLabel('🌸 More Puffies!')
        ...
        console.error('Error fetching puffies image:', error);
        const errorPayload = createApiErrorPayload("puffies");
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('puffies')
        .setDescription('Delivers images of delicate puffy nipples. 🌸'),
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generatePuffiesPayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generatePuffiesPayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {await interaction.deferUpdate();
                const newPayload = await generatePuffiesPayload(interaction);
                // Check if generatePuffiesPayload returned an error payload
                ...
                }catch (error) {console.error('Error handling puffies reload button:', error);
                // General error handling for other issues, send ephemeral error
                const errorPayload = createApiErrorPayload("puffies"); // Use the updated error payload function
                 if (interaction.replied || interaction.deferred) {

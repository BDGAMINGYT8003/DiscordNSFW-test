...
const imagePreloader = new ImagePreloader("dick");
const cooldowns = new Map(); // UserID -> timestamp

// Main function to generate the image payload
const generateDickPayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0x006994) // A sturdy blue for dick
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### Behold! A Magnificent Specimen! 🍆')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('A fine phallic view.')
                )
            );

        const reloadButton = new ButtonBuilder()
            .setCustomId('dick_button_reload')
            .setLabel('🍆 More Dicks!')
        ...
        console.error('Error fetching dick image:', error);
        const errorPayload = createApiErrorPayload("dick");
...
module.exports = {
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generateDickPayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generateDickPayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {await interaction.deferUpdate();
                const newPayload = await generateDickPayload(interaction);
                // Check if generateDickPayload returned an error payload
            ...
            }  catch (error) {console.error('Error handling dick reload button:', error);
                // General error handling for other issues, send ephemeral error
                const errorPayload = createApiErrorPayload("dick"); // Use the updated error payload function
                 if (interaction.replied || interaction.deferred) {

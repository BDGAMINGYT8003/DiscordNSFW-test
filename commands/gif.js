...
const imagePreloader = new ImagePreloader("gif");
const cooldowns = new Map(); // UserID -> timestamp

// Main function to generate the image payload
const generateGifPayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0x00FFFF) // Cyan for GIF / animation
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### Action Packed GIF! 🎬')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('A delightful animated GIF.')
                )
            );

        const reloadButton = new ButtonBuilder()
            .setCustomId('gif_button_reload')
            .setLabel('🎬 More GIFs!')
        ...
        const errorPayload = createApiErrorPayload("gif");
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('gif')
        .setDescription('Delivers delightful animated GIFs. 🎬'),
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generateGifPayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generateGifPayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {await interaction.deferUpdate();
                const newPayload = await generateGifPayload(interaction);
                // Check if generateGifPayload returned an error payload
                ...
                }catch (error) {console.error('Error handling gif reload button:', error);
                // General error handling for other issues, send ephemeral error
                const errorPayload = createApiErrorPayload("gif"); // Use the updated error payload function
                 if (interaction.replied || interaction.deferred) {

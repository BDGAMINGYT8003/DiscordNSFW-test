...
const imagePreloader = new ImagePreloader("tiny");
const cooldowns = new Map(); // UserID -> timestamp

// Main function to generate the image payload
const generateTinyPayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0xFFE4E1) // MistyRose for tiny
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### Petite and Precious! ✨🤏')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('A delightful view of something tiny.')
                )
            );

        const reloadButton = new ButtonBuilder()
            .setCustomId('tiny_button_reload')
            .setLabel('🤏 More Tiny Treasures!')
        ...
        console.error('Error fetching tiny image:', error);
        const errorPayload = createApiErrorPayload("tiny");
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('tiny')
        .setDescription('Delivers images of tiny and petite subjects. ✨🤏'),
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generateTinyPayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generateTinyPayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {await interaction.deferUpdate();
                const newPayload = await generateTinyPayload(interaction);
                // Check if generateTinyPayload returned an error payload
            ...
            }  catch (error) {console.error('Error handling tiny reload button:', error);
                // General error handling for other issues, send ephemeral error
                const errorPayload = createApiErrorPayload("tiny"); // Use the updated error payload function
                 if (interaction.replied || interaction.deferred) {

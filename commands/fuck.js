...
const CATEGORY = "fuck"; // Define category for easy reuse
...
const createCooldownEmbed = (timeLeft) => { // Renamed and uses EmbedBuilder
    return new EmbedBuilder()
        ...
        .setTitle('⏰ Easy There, Tiger!')
        .setDescription(`You must wait **${timeLeft.toFixed(1)}s** before initiating another passionate encounter. Even Rome wasn't built in a day!`)
        .setTimestamp();
};
...
const generateImageEmbedAndComponents = async () => {
    ...
    const embed = new EmbedBuilder()
        .setColor(0xDC143C) // Crimson for fuck
        .setTitle('Passionate Encounters! 🔥')
        .setImage(imageUrl)
        .setTimestamp()
        .setFooter({ text: `Category: ${CATEGORY}` });

    const reloadButton = new ButtonBuilder()
        .setCustomId(`${CATEGORY}_button_reload`) // e.g., fuck_button_reload
...
module.exports = {
    ...
    async handleComponent(interaction, componentArgs) {
        // componentArgs: ['button', 'reload'] from customId like 'fuck_button_reload'
        if (!interaction.channel || !interaction.channel.nsfw) {
            return interaction.reply({ embeds: [createNsfwOnlyEmbed()], ephemeral: true });
        }

...
const imagePreloader = new ImagePreloader("fuck");
const cooldowns = new Map(); // UserID -> timestamp

// Main function to generate the image payload
const generateFuckPayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0xDC143C) // Crimson for fuck
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### Passionate Encounters! 🔥')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('A very intimate and passionate moment.')
                )
            );

        const reloadButton = new ButtonBuilder()
            .setCustomId('fuck_button_reload')
            .setLabel('🔥 More Fucking!')
        ...
        console.error('Error fetching fuck image:', error);
        const errorPayload = createApiErrorPayload("fuck");
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('fuck')
        .setDescription('Delivers images of passionate fucking. 🔥'),
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generateFuckPayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generateFuckPayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {await interaction.deferUpdate();
                const newPayload = await generateFuckPayload(interaction);
                // Check if generateFuckPayload returned an error payload
                ...
                }catch (error) {console.error('Error handling fuck reload button:', error);
                // General error handling for other issues, send ephemeral error
                const errorPayload = createApiErrorPayload("fuck"); // Use the updated error payload function
                 if (interaction.replied || interaction.deferred) {

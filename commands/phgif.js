...
const imagePreloader = new ImagePreloader("phgif");
const cooldowns = new Map(); // UserID -> timestamp

// Main function to generate the image payload
const generatePhgifPayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0xFFA500) // Orange for PHGIF
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### Premium Hub GIF Action! 🎬🧡')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('A quality animated GIF from the Hub.')
                )
            );

        const reloadButton = new ButtonBuilder()
            .setCustomId('phgif_button_reload')
            .setLabel('🧡 More PH GIFs!')
        ...
        console.error('Error fetching phgif image:', error);
        const errorPayload = createApiErrorPayload("phgif");
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('phgif')
        .setDescription('Delivers quality animated GIFs from the Hub. 🎬🧡'),
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generatePhgifPayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generatePhgifPayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {await interaction.deferUpdate();
                const newPayload = await generatePhgifPayload(interaction);
                // Check if generatePhgifPayload returned an error payload
            ...
            }  catch (error) {console.error('Error handling phgif reload button:', error);
                // General error handling for other issues, send ephemeral error
                const errorPayload = createApiErrorPayload("phgif"); // Use the updated error payload function
                 if (interaction.replied || interaction.deferred) {

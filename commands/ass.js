    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {
                ...
                const actionRow = container.components.find(c => c.components && c.components[0].custom_id === 'ass_button_reload');
                if (actionRow) {
                ...
                const countdownInterval = setInterval(async () => {
                    countdown--;
                    if (countdown > 0) {
                        const updatedPayload = await generateAssPayload();
                        ...
                        const finalPayload = await generateAssPayload();

...
const generateAssPayload = async (useCache = false) => {
    try {
        let imageUrl;
        
        if (useCache) {
            imageUrl = await getCachedImage('ass');
            if (!imageUrl) {
                imageUrl = await imagePreloader.getImage();
                await setCachedImage('ass', imageUrl);
            }
        } else {
...
const updateButtonWithCountdown = async (interaction, remainingTime) => {
    const cachedImageUrl = await getCachedImage('ass');
...
module.exports = {
    ...
    async slashExecute(interaction) {
        ...
        if (await isOnCooldown(userId, 'ass')) {
            const embed = new EmbedBuilder()
                .setColor(0xFF0000)
                .setTitle('⏰ Hold your horses there, viewer!')
                .setDescription('The stunning display needs a moment to prepare its next pose. Please wait before requesting another captivating view.')
                .setTimestamp();
            
            return await interaction.reply({ embeds: [embed], ephemeral: true });
        }

        await setCooldown(userId, 'ass');
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            if (await isOnCooldown(userId, 'ass')) {
                const embed = new EmbedBuilder()
                    .setColor(0xFF0000)
                    .setTitle('⏰ Patience, eager spectator!')
                    .setDescription('The beautiful anatomy needs time to strike another alluring pose. Wait a moment before requesting another mesmerizing display.')
                    .setTimestamp();
            ...
            try {
                ...
                const countdownInterval = setInterval(async () => {
                    const cooldowns = await readJsonFile(cooldownFile);
                    const userCooldowns = cooldowns[userId] || {};
                    const lastUsed = userCooldowns['ass'];

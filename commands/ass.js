...
const CATEGORY = "ass"; // Define category for easy reuse and to match command name
...
const createCooldownEmbed = (timeLeft) => { // Renamed and uses EmbedBuilder
    return new EmbedBuilder()
        ...
        .setTitle('⏰ Patience, Admirer')
        .setDescription(`You must wait **${timeLeft.toFixed(1)}s** before viewing another stunning display. Art requires contemplation.`)
        .setTimestamp();
};

// This function creates an embed for API errors.
...
class ImagePreloader {
    ...
    async fetchAndCache() {
        try {
            const data = await nsfw.fetch(this.category); // Uses this.category (which is CATEGORY constant)
...
const generateImageEmbedAndComponents = async () => {
    ...
    const embed = new EmbedBuilder()
        .setColor(0xE6B800) // A golden yellow for ass
        .setTitle('Behold! A Magnificent Ass! 🍑')
        .setImage(imageUrl)
        .setTimestamp()
        .setFooter({ text: `Category: ${CATEGORY}` });

    const reloadButton = new ButtonBuilder()
        .setCustomId(`${CATEGORY}_button_reload`) // e.g., ass_button_reload
        .setLabel('🔃 More Ass!')
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName(CATEGORY) // Use the CATEGORY constant
        .setDescription('Offers a glimpse of delightful gluteal anatomy. 🍑'), // Removed (NSFW channels only)
    ...
    async handleComponent(interaction, componentArgs) {
        // componentArgs: ['button', 'reload'] from customId like 'ass_button_reload'
        if (!interaction.channel || !interaction.channel.nsfw) {
            return interaction.reply({ embeds: [createNsfwOnlyEmbed()], ephemeral: true });
        }

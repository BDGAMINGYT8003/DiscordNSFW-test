...
const generateBoobsPayload = async (cachedUrl = null) => {
    try {
...
module.exports = {
    ...
    async slashExecute(interaction) {
        const cooldownCheck = checkCooldown(interaction.user.id, 'boobs');
        ...
        setCooldown(interaction.user.id, 'boobs');
    ...
    async prefixExecute(message, args) {
        const cooldownCheck = checkCooldown(message.author.id, 'boobs');
        ...
        // Set cooldown after successful execution
        setCooldown(message.author.id, 'boobs');
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {
                ...
                const cache = loadCache();
                const cachedUrl = cache.lastBoobsUrl;

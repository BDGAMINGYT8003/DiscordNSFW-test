...
const CATEGORY = "boobs"; // Define category for easy reuse
...
const createCooldownEmbed = (timeLeft) => { // Renamed and uses EmbedBuilder
    return new EmbedBuilder()
        ...
        .setTitle('⏰ Patience, Devoted Admirer')
        .setDescription(`You must wait **${timeLeft.toFixed(1)}s** before beholding another sight. Beauty demands reverence.`)
        .setTimestamp();
};
...
const generateImageEmbedAndComponents = async () => {
    ...
    const embed = new EmbedBuilder()
        .setColor(0xFF69B4) // Hot pink for boobs
        .setTitle('Behold! A Magnificent Pair! 🍈🍈')
        .setImage(imageUrl)
        .setTimestamp()
        .setFooter({ text: `Category: ${CATEGORY}` });

    const reloadButton = new ButtonBuilder()
        .setCustomId(`${CATEGORY}_button_reload`) // e.g., boobs_button_reload
...
module.exports = {
    ...
    async handleComponent(interaction, componentArgs) {
        // componentArgs: ['button', 'reload'] from customId like 'boobs_button_reload'
        if (!interaction.channel || !interaction.channel.nsfw) {
            return interaction.reply({ embeds: [createNsfwOnlyEmbed()], ephemeral: true });
        }
        ...
        const userId = interaction.user.id;
        const now = Date.now();
        const cooldownAmount = 2000; // Shorter cooldown

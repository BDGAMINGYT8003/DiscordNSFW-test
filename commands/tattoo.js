...
const imagePreloader = new ImagePreloader("tattoo");
const cooldowns = new Map(); // UserID -> timestamp

// Main function to generate the image payload
const generateTattooPayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0x2F4F4F) // DarkSlateGray for tattoo
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### Inked and Beautiful! 🎨✒️')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('A stunning display of body art.')
                )
            );

        const reloadButton = new ButtonBuilder()
            .setCustomId('tattoo_button_reload')
            .setLabel('✒️ More Tattoos!')
        ...
        console.error('Error fetching tattoo image:', error);
        const errorPayload = createApiErrorPayload("tattoo");
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('tattoo')
        .setDescription('Delivers images of beautiful tattoos. 🎨✒️'),
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generateTattooPayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generateTattooPayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {await interaction.deferUpdate();
                const newPayload = await generateTattooPayload(interaction);
                // Check if generateTattooPayload returned an error payload
                ...
                }catch (error) {console.error('Error handling tattoo reload button:', error);
                // General error handling for other issues, send ephemeral error
                const errorPayload = createApiErrorPayload("tattoo"); // Use the updated error payload function
                 if (interaction.replied || interaction.deferred) {

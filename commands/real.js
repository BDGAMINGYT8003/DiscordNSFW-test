...
const imagePreloader = new ImagePreloader("real");
const cooldowns = new Map(); // UserID -> timestamp

// Main function to generate the image payload
const generateRealPayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0x778899) // LightSlateGray for 'real'
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### Keeping it Real!  autentico 👀')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('An authentic, real view.')
                )
            );

        const reloadButton = new ButtonBuilder()
            .setCustomId('real_button_reload')
            .setLabel('👀 More Realness!')
        ...
        const errorPayload = createApiErrorPayload("real");
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('real')
        .setDescription('Delivers authentic "real" images. 👀'),
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generateRealPayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generateRealPayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {await interaction.deferUpdate();
                const newPayload = await generateRealPayload(interaction);
                // Check if generateRealPayload returned an error payload
                ...
                }catch (error) {console.error('Error handling real reload button:', error);
                // General error handling for other issues, send ephemeral error
                const errorPayload = createApiErrorPayload("real"); // Use the updated error payload function
                 if (interaction.replied || interaction.deferred) {

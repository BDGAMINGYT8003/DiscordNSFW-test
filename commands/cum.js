...
const generateCumPayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0xF5F5F5) // Off-white for cum
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### A Sticky Situation! 💦')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('A delightful mess.')
                )
            );
        ...
        const errorPayload = createApiErrorPayload("cum");
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('cum')
        .setDescription('Delivers sticky situations. 💦'),
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generateCumPayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generateCumPayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {await interaction.deferUpdate();
                const newPayload = await generateCumPayload(interaction);
                // Check if generateCumPayload returned an error payload
                ...
                }catch (error) {console.error('Error handling cum reload button:', error);
                // General error handling for other issues, send ephemeral error
                const errorPayload = createApiErrorPayload("cum"); // Use the updated error payload function
                 if (interaction.replied || interaction.deferred) {

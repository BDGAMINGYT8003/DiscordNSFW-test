...
const imagePreloader = new ImagePreloader("cum");

// Shared function to generate the response payload using Components V2
const generateCumPayload = async (cachedUrl = null) => {
    try {
        ...
        const container = new ContainerBuilder()
            ...
            .addTextDisplayComponents( // Add a title
                textDisplay => textDisplay
                    .setContent('### Behold! The climactic moment.') // Markdown for a nice heading
            )
                ...
                mediaGallery => mediaGallery
                    .addItems( // Add the image/media from the fetched URL
                        mediaGalleryItem => mediaGalleryItem
                            ...
                            .setURL(imageUrl)
                            .setDescription('The final release.') // Alt text.
                    )
            );

        // Add the Reload button.
        const reloadButton = new ButtonBuilder()
            .setCustomId('cum_button_reload') // Unique ID for this command's reload button.
            .setLabel('🔃 Reload')
        ...
        console.error('Error fetching cum image:', error);
        ...
        const errorContainer = new ContainerBuilder()
            ...
            .addTextDisplayComponents(
                textDisplay => textDisplay
                    .setContent('Failed to fetch the requested image. The moment seems to have passed.')
            );
...
const updateButtonCountdown = async (interaction, seconds, cachedUrl) => {
    const container = new ContainerBuilder()
        ...
        .addTextDisplayComponents(
            textDisplay => textDisplay
                .setContent('### Behold! The climactic moment.')
        )
        .addSeparatorComponents(
            separator => separator
        ...
        .addMediaGalleryComponents(
            mediaGallery => mediaGallery
                .addItems(
                    mediaGalleryItem => mediaGalleryItem
                        .setURL(cachedUrl)
                        .setDescription('The final release.')
                )
        );

    const reloadButton = new ButtonBuilder()
        .setCustomId('cum_button_reload')
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('cum')
        .setDescription('Captures moments of intense pleasure.'), // A descriptive explanation.

    // Slash Command Execution
    async slashExecute(interaction) {
        const cooldownCheck = checkCooldown(interaction.user.id, 'cum');
        ...
        const payload = await generateCumPayload();
        
        // Set cooldown after successful execution
        setCooldown(interaction.user.id, 'cum');
    ...
    async prefixExecute(message, args) {
        const cooldownCheck = checkCooldown(message.author.id, 'cum');
        ...
        // Set cooldown after successful execution
        setCooldown(message.author.id, 'cum');
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {
                ...
                const cache = loadCache();
                const cachedUrl = cache.lastCumUrl;
                ...
                const newPayload = await generateCumPayload();

                // Edit the original message.
                await interaction.editReply(newPayload);

            } catch (error) {
                console.error('Error handling cum reload button:', error);

...
const generateBottomlessPayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0xADD8E6) // Light blue for bottomless
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### Feeling Free and Bottomless! 🌬️')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('A liberating bottomless view.')
                )
            );

        const reloadButton = new ButtonBuilder()
            .setCustomId('bottomless_button_reload')
            .setLabel('🌬️ More Freedom!')
        ...
        const errorPayload = createApiErrorPayload("bottomless");
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('bottomless')
        .setDescription('Delivers liberating bottomless images. 🌬️ (NSFW channels only)'),
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generateBottomlessPayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generateBottomlessPayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {
                ...
                const newPayload = await generateBottomlessPayload(interaction);

...
const imagePreloader = new ImagePreloader("bottomless");

// Shared function to generate the response payload using Components V2
const generateBottomlessPayload = async (cachedUrl = null) => {
    try {
        ...
        if (!cachedUrl) {
            ...
            cache.lastBottomlessUrl = imageUrl;
            saveCache(cache);
        }
        ...
        const container = new ContainerBuilder()
            ...
            .addTextDisplayComponents( // Add a title
                textDisplay => textDisplay
                    .setContent('### Behold! A liberating display.') // Markdown for a nice heading
            )
                ...
                mediaGallery => mediaGallery
                    .addItems( // Add the image/media from the fetched URL
                        mediaGalleryItem => mediaGalleryItem
                            ...
                            .setURL(imageUrl)
                            .setDescription('Freedom from below.') // Alt text.
                    )
            );

        // Add the Reload button.
        const reloadButton = new ButtonBuilder()
            .setCustomId('bottomless_button_reload') // Unique ID for this command's reload button.
            .setLabel('🔃 Reload')
        ...
        console.error('Error fetching bottomless image:', error);
        ...
        const errorContainer = new ContainerBuilder()
            ...
            .addTextDisplayComponents(
                textDisplay => textDisplay
                    .setContent('Failed to fetch the requested image. The lower half seems to be hiding.')
            );
...
const updateButtonCountdown = async (interaction, seconds, cachedUrl) => {
    const container = new ContainerBuilder()
        ...
        .addTextDisplayComponents(
            textDisplay => textDisplay
                .setContent('### Behold! A liberating display.')
        )
        .addSeparatorComponents(
            separator => separator
        ...
        .addMediaGalleryComponents(
            mediaGallery => mediaGallery
                .addItems(
                    mediaGalleryItem => mediaGalleryItem
                        .setURL(cachedUrl)
                        .setDescription('Freedom from below.')
                )
        );

    const reloadButton = new ButtonBuilder()
        .setCustomId('bottomless_button_reload')
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('bottomless')
        .setDescription('Showcases freedom below the waist.'), // A descriptive explanation.

    // Slash Command Execution
    async slashExecute(interaction) {
        const cooldownCheck = checkCooldown(interaction.user.id, 'bottomless');
        ...
        const payload = await generateBottomlessPayload();
        
        // Set cooldown after successful execution
        setCooldown(interaction.user.id, 'bottomless');
    ...
    async prefixExecute(message, args) {
        const cooldownCheck = checkCooldown(message.author.id, 'bottomless');
        ...
        // Set cooldown after successful execution
        setCooldown(message.author.id, 'bottomless');
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {
                ...
                const cache = loadCache();
                const cachedUrl = cache.lastBottomlessUrl;
                ...
                const newPayload = await generateBottomlessPayload();

                // Edit the original message.
                await interaction.editReply(newPayload);

            } catch (error) {
                console.error('Error handling bottomless reload button:', error);

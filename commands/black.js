...
const imagePreloader = new ImagePreloader("black");
const cooldowns = new Map(); // UserID -> timestamp

// Main function to generate the image payload
const generateBlackPayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0x303030) // Dark gray / near black for black
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### Behold! Ebony Beauty! 🖤')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('A stunning view of ebony artistry.')
                )
            );

        const reloadButton = new ButtonBuilder()
            .setCustomId('black_button_reload')
            .setLabel('🖤 More Ebony!')
            .setStyle(ButtonStyle.Secondary); // Secondary for a sleek look
        ...
        console.error('Error fetching black image:', error);
        const errorPayload = createApiErrorPayload("black");
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('black')
        .setDescription('Delivers delightful ebony images. 🖤'),
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generateBlackPayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generateBlackPayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {await interaction.deferUpdate();
                const newPayload = await generateBlackPayload(interaction);
                // Check if generateBlackPayload returned an error payload
            ...
            }  catch (error) {console.error('Error handling black reload button:', error);
                // General error handling for other issues, send ephemeral error
                const errorPayload = createApiErrorPayload("black"); // Use the updated error payload function
                 if (interaction.replied || interaction.deferred) {

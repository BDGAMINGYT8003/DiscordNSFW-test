...
const imagePreloader = new ImagePreloader("dom");
const cooldowns = new Map(); // UserID -> timestamp

// Main function to generate the image payload
const generateDomPayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0x4B0082) // Indigo for Dom
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### In Control and Dominant! 👑')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('A display of sheer dominance.')
                )
            );

        const reloadButton = new ButtonBuilder()
            .setCustomId('dom_button_reload')
            .setLabel('👑 More Dominance!')
            .setStyle(ButtonStyle.Secondary); // Secondary for a commanding presence
        ...
        console.error('Error fetching dom image:', error);
        const errorPayload = createApiErrorPayload("dom");
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('dom')
        .setDescription('Delivers images of dominant figures. 👑'),
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generateDomPayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generateDomPayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {await interaction.deferUpdate();
                const newPayload = await generateDomPayload(interaction);
                // Check if generateDomPayload returned an error payload
            ...
            }  catch (error) {console.error('Error handling dom reload button:', error);
                // General error handling for other issues, send ephemeral error
                const errorPayload = createApiErrorPayload("dom"); // Use the updated error payload function
                 if (interaction.replied || interaction.deferred) {

...
const generateBdsmPayload = async (cachedUrl = null) => {
    try {
...
module.exports = {
    ...
    async slashExecute(interaction) {
        const cooldownCheck = checkCooldown(interaction.user.id, 'bdsm');
        ...
        setCooldown(interaction.user.id, 'bdsm');
    ...
    async prefixExecute(message, args) {
        const cooldownCheck = checkCooldown(message.author.id, 'bdsm');
        ...
        // Set cooldown after successful execution
        setCooldown(message.author.id, 'bdsm');
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {
                ...
                const cache = loadCache();
                const cachedUrl = cache.lastBdsmUrl;

...
const createCooldownPayload = (timeLeft) => {
    const container = new ContainerBuilder()
        ...
        .addTextDisplayComponents(
            textDisplay => textDisplay.setContent('### ⏰ Patience, Seeker of Control'),
            textDisplay => textDisplay.setContent(`You must wait **${timeLeft.toFixed(1)}s** before exploring another scene of power. Control requires patience.`)
        )
        .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small));
    return { components: [container], flags: MessageFlags.IsComponentsV2, ephemeral: true };
};
...
const createApiErrorPayload = (category) => {
    const container = new ContainerBuilder()
        ...
        .addTextDisplayComponents(
            ...
            textDisplay => textDisplay.setContent(`Failed to fetch an image for the **${category}** category. The scene might be too intense for the API right now. Please try again later. The NSFWHub API we use to fetch images is very strictly rate-limited, so the most likely reason for the error is due to this limitation.`)
        )
        .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small));
    return { components: [container], flags: MessageFlags.IsComponentsV2, ephemeral: true };
};
...
const generateBdsmPayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0x8B0000) // Dark red for BDSM
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### An Exploration of Control & Desire... ⛓️')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('Ties that bind, pleasures defined.')
                )
            );

        const reloadButton = new ButtonBuilder()
            .setCustomId('bdsm_button_reload')
            .setLabel('⛓️ More Discipline!')
            .setStyle(ButtonStyle.Secondary); // Secondary, perhaps for a more "subtle" control
        ...
        const errorPayload = createApiErrorPayload("bdsm");
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('bdsm')
        .setDescription('Explores themes of power, pleasure, and discipline. ⛓️'),
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generateBdsmPayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generateBdsmPayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {await interaction.deferUpdate();
                const newPayload = await generateBdsmPayload(interaction);
                // Check if generateBdsmPayload returned an error payload
                ...
                }catch (error) {console.error('Error handling bdsm reload button:', error);
                // General error handling for other issues, send ephemeral error
                const errorPayload = createApiErrorPayload("bdsm"); // Use the updated error payload function
                 if (interaction.replied || interaction.deferred) {

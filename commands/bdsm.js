...
const generateBdsmPayload = async (useCache = false) => {
    try {
        let imageUrl;
        
        if (useCache) {
            imageUrl = await getCachedImage('bdsm');
            if (!imageUrl) {
                imageUrl = await imagePreloader.getImage();
                await setCachedImage('bdsm', imageUrl);
            }
        } else {
...
const updateButtonWithCountdown = async (interaction, remainingTime) => {
    const cachedImageUrl = await getCachedImage('bdsm');
    ...
    const container = new ContainerBuilder()
        ...
        .addTextDisplayComponents(
            textDisplay => textDisplay
                .setContent('### Behold! An exploration of control.')
        )
        .addSeparatorComponents(
            separator => separator
...
module.exports = {
    ...
    async slashExecute(interaction) {
        ...
        if (await isOnCooldown(userId, 'bdsm')) {
            const embed = new EmbedBuilder()
                .setColor(0xFF0000)
                .setTitle('⏰ Patience, devoted one!')
                .setDescription('The scene requires time to prepare. True dominance cannot be rushed. Please wait a moment before requesting another exploration.')
                .setTimestamp();
            
            return await interaction.reply({ embeds: [embed], ephemeral: true });
        }

        await setCooldown(userId, 'bdsm');
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            if (await isOnCooldown(userId, 'bdsm')) {
                const embed = new EmbedBuilder()
                    .setColor(0xFF0000)
                    .setTitle('⏰ Control yourself, eager submissive!')
                    .setDescription('The master\'s next command requires patience. Wait for the scene to be properly prepared before requesting another display of power.')
                    .setTimestamp();
            ...
            try {
                ...
                const countdownInterval = setInterval(async () => {
                    const cooldowns = await readJsonFile(cooldownFile);
                    const userCooldowns = cooldowns[userId] || {};
                    const lastUsed = userCooldowns['bdsm'];

...
const CATEGORY = "bdsm"; // Define category for easy reuse
...
const createCooldownEmbed = (timeLeft) => { // Renamed and uses EmbedBuilder
    return new EmbedBuilder()
        ...
        .setTitle('⏰ Patience, Seeker of Control')
        .setDescription(`You must wait **${timeLeft.toFixed(1)}s** before exploring another scene of power. Control requires patience.`)
        .setTimestamp();
};
...
class ImagePreloader {
    constructor(category) {
        this.category = category; // Should be CATEGORY constant passed in
        this.cache = [];
...
const generateImageEmbedAndComponents = async () => {
    ...
    const embed = new EmbedBuilder()
        .setColor(0x8B0000) // Dark red for BDSM
        .setTitle('An Exploration of Control & Desire... ⛓️')
        .setImage(imageUrl)
        .setTimestamp()
        .setFooter({ text: `Category: ${CATEGORY}` });

    const reloadButton = new ButtonBuilder()
        .setCustomId(`${CATEGORY}_button_reload`) // e.g., bdsm_button_reload
...
module.exports = {
    ...
    async handleComponent(interaction, componentArgs) {
        // componentArgs: ['button', 'reload'] from customId like 'bdsm_button_reload'
        if (!interaction.channel || !interaction.channel.nsfw) {
            return interaction.reply({ embeds: [createNsfwOnlyEmbed()], ephemeral: true });
        }

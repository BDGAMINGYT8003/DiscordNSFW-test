...
const imagePreloader = new ImagePreloader("dp");
const cooldowns = new Map(); // UserID -> timestamp

// Main function to generate the image payload
const generateDpPayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0x800080) // Purple for DP
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### Double the Fun! ✌️💦')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('A truly fulfilling experience.')
                )
            );

        const reloadButton = new ButtonBuilder()
            .setCustomId('dp_button_reload')
            .setLabel('✌️ More DP Action!')
        ...
        console.error('Error fetching dp image:', error);
        const errorPayload = createApiErrorPayload("dp");
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('dp')
        .setDescription('Delivers double penetration images. ✌️💦'),
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generateDpPayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generateDpPayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {await interaction.deferUpdate();
                const newPayload = await generateDpPayload(interaction);
                // Check if generateDpPayload returned an error payload
                ...
                }catch (error) {console.error('Error handling dp reload button:', error);
                // General error handling for other issues, send ephemeral error
                const errorPayload = createApiErrorPayload("dp"); // Use the updated error payload function
                 if (interaction.replied || interaction.deferred) {

...
const imagePreloader = new ImagePreloader("cumsluts");

// Shared function to generate the response payload using Components V2
const generateCumslutsPayload = async (cachedUrl = null) => {
    try {
        ...
        if (!cachedUrl) {
            ...
            cache.lastCumslutsUrl = imageUrl;
            saveCache(cache);
        }
        ...
        const container = new ContainerBuilder()
            ...
            .addTextDisplayComponents( // Add a title
                textDisplay => textDisplay
                    .setContent('### Behold! Devoted enthusiasts.') // Markdown for a nice heading
            )
                ...
                mediaGallery => mediaGallery
                    .addItems( // Add the image/media from the fetched URL
                        mediaGalleryItem => mediaGalleryItem
                            ...
                            .setURL(imageUrl)
                            .setDescription('Pure dedication.') // Alt text.
                    )
            );

        // Add the Reload button.
        const reloadButton = new ButtonBuilder()
            .setCustomId('cumsluts_button_reload') // Unique ID for this command's reload button.
            .setLabel('🔃 Reload')
        ...
        console.error('Error fetching cumsluts image:', error);
        ...
        const errorContainer = new ContainerBuilder()
            ...
            .addTextDisplayComponents(
                textDisplay => textDisplay
                    .setContent('Failed to fetch the requested image. The devotees seem to be taking a break.')
            );
...
const updateButtonCountdown = async (interaction, seconds, cachedUrl) => {
    const container = new ContainerBuilder()
        ...
        .addTextDisplayComponents(
            textDisplay => textDisplay
                .setContent('### Behold! Devoted enthusiasts.')
        )
        .addSeparatorComponents(
            separator => separator
        ...
        .addMediaGalleryComponents(
            mediaGallery => mediaGallery
                .addItems(
                    mediaGalleryItem => mediaGalleryItem
                        .setURL(cachedUrl)
                        .setDescription('Pure dedication.')
                )
        );

    const reloadButton = new ButtonBuilder()
        .setCustomId('cumsluts_button_reload')
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('cumsluts')
        .setDescription('Features dedicated pleasure seekers.'), // A descriptive explanation.

    // Slash Command Execution
    async slashExecute(interaction) {
        const cooldownCheck = checkCooldown(interaction.user.id, 'cumsluts');
        ...
        const payload = await generateCumslutsPayload();
        
        // Set cooldown after successful execution
        setCooldown(interaction.user.id, 'cumsluts');
    ...
    async prefixExecute(message, args) {
        const cooldownCheck = checkCooldown(message.author.id, 'cumsluts');
        ...
        // Set cooldown after successful execution
        setCooldown(message.author.id, 'cumsluts');
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {
                ...
                const cache = loadCache();
                const cachedUrl = cache.lastCumslutsUrl;
                ...
                const newPayload = await generateCumslutsPayload();

                // Edit the original message.
                await interaction.editReply(newPayload);

            } catch (error) {
                console.error('Error handling cumsluts reload button:', error);

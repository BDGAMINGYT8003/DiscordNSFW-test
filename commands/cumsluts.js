...
const generateCumslutsPayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0xFF1493) // Deep pink for cumsluts
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### Eager and Ready! 👄💦')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('Always happy to please.')
                )
            );

        const reloadButton = new ButtonBuilder()
            .setCustomId('cumsluts_button_reload')
            .setLabel('💦 More Eager Faces!')
            .setStyle(ButtonStyle.Danger); // Danger for the intensity
        ...
        const errorPayload = createApiErrorPayload("cumsluts");
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('cumsluts')
        .setDescription('Delivers images of eager individuals. 👄💦'),
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generateCumslutsPayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generateCumslutsPayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {await interaction.deferUpdate();
                const newPayload = await generateCumslutsPayload(interaction);
                // Check if generateCumslutsPayload returned an error payload
                ...
                }catch (error) {console.error('Error handling cumsluts reload button:', error);
                // General error handling for other issues, send ephemeral error
                const errorPayload = createApiErrorPayload("cumsluts"); // Use the updated error payload function
                 if (interaction.replied || interaction.deferred) {

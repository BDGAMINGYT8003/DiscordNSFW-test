...
const imagePreloader = new ImagePreloader("feet");

// Shared function to generate the response payload using Components V2
const generateFeetPayload = async (cachedUrl = null) => {
    try {
        ...
        const container = new ContainerBuilder()
            ...
            .addTextDisplayComponents( // Add a title
                textDisplay => textDisplay
                    .setContent('### Behold! Steps of elegance.') // Markdown for a nice heading
            )
                ...
                mediaGallery => mediaGallery
                    .addItems( // Add the image/media from the fetched URL
                        mediaGalleryItem => mediaGalleryItem
                            ...
                            .setURL(imageUrl)
                            .setDescription('Walking in beauty.') // Alt text.
                    )
            );

        // Add the Reload button.
        const reloadButton = new ButtonBuilder()
            .setCustomId('feet_button_reload') // Unique ID for this command's reload button.
            .setLabel('🔃 Reload')
        ...
        const errorContainer = new ContainerBuilder()
            ...
            .addTextDisplayComponents(
                textDisplay => textDisplay
                    .setContent('Failed to fetch the requested image. The steps have gone astray.')
            );
...
const updateButtonCountdown = async (interaction, seconds, cachedUrl) => {
    const container = new ContainerBuilder()
        ...
        .addTextDisplayComponents(
            textDisplay => textDisplay
                .setContent('### Behold! Steps of elegance.')
        )
        .addSeparatorComponents(
            separator => separator
        ...
        .addMediaGalleryComponents(
            mediaGallery => mediaGallery
                .addItems(
                    mediaGalleryItem => mediaGalleryItem
                        .setURL(cachedUrl)
                        .setDescription('Walking in beauty.')
                )
        );

    const reloadButton = new ButtonBuilder()
        .setCustomId('feet_button_reload')
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('feet')
        .setDescription('Showcases the beauty of footwork.'), // A descriptive explanation.

    // Slash Command Execution
    async slashExecute(interaction) {
        const cooldownCheck = checkCooldown(interaction.user.id, 'feet');
        ...
        const payload = await generateFeetPayload();
        
        // Set cooldown after successful execution
        setCooldown(interaction.user.id, 'feet');
    ...
    async prefixExecute(message, args) {
        const cooldownCheck = checkCooldown(message.author.id, 'feet');
        ...
        // Set cooldown after successful execution
        setCooldown(message.author.id, 'feet');
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {
                ...
                const cache = loadCache();
                const cachedUrl = cache.lastFeetUrl;
                ...
                const newPayload = await generateFeetPayload();

                // Edit the original message.
                await interaction.editReply(newPayload);

            } catch (error) {
                console.error('Error handling feet reload button:', error);

...
const CATEGORY = "xmas"; // Define category for easy reuse
...
const createCooldownEmbed = (timeLeft) => { // Renamed and uses EmbedBuilder
    return new EmbedBuilder()
        ...
        .setTitle('⏰ Hold Your Reindeer!')
        .setDescription(`You must wait **${timeLeft.toFixed(1)}s** before unwrapping another Christmas surprise. Santa needs a break!`)
        .setTimestamp();
};
...
const generateImageEmbedAndComponents = async () => {
    ...
    const embed = new EmbedBuilder()
        .setColor(0x008000) // Green for Xmas
        .setTitle('Naughty Christmas Surprise! 🎄🎁')
        .setImage(imageUrl)
        .setTimestamp()
        .setFooter({ text: `Category: ${CATEGORY}` });

    const reloadButton = new ButtonBuilder()
        .setCustomId(`${CATEGORY}_button_reload`) // e.g., xmas_button_reload
...
module.exports = {
    ...
    async handleComponent(interaction, componentArgs) {
        // componentArgs: ['button', 'reload'] from customId like 'xmas_button_reload'
        if (!interaction.channel || !interaction.channel.nsfw) {
            return interaction.reply({ embeds: [createNsfwOnlyEmbed()], ephemeral: true });
        }

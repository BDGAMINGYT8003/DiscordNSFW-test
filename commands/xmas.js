...
const imagePreloader = new ImagePreloader("xmas");
const cooldowns = new Map(); // UserID -> timestamp

// Main function to generate the image payload
const generateXmasPayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0x008000) // Green for Xmas
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### Naughty Christmas Surprise! 🎄🎁')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('A festive and naughty Xmas treat.')
                )
            );

        const reloadButton = new ButtonBuilder()
            .setCustomId('xmas_button_reload')
            .setLabel('🎁 More Xmas Gifts!')
            .setStyle(ButtonStyle.Success); // Success for a gift!
        ...
        console.error('Error fetching xmas image:', error);
        const errorPayload = createApiErrorPayload("xmas");
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('xmas')
        .setDescription('Delivers naughty Christmas themed images. 🎄🎁'),
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generateXmasPayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generateXmasPayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {await interaction.deferUpdate();
                const newPayload = await generateXmasPayload(interaction);
                // Check if generateXmasPayload returned an error payload
                ...
                }catch (error) {console.error('Error handling xmas reload button:', error);
                // General error handling for other issues, send ephemeral error
                const errorPayload = createApiErrorPayload("xmas"); // Use the updated error payload function
                 if (interaction.replied || interaction.deferred) {

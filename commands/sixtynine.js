...
const createApiErrorPayload = (category) => {
    const container = new ContainerBuilder()
        ...
        .addTextDisplayComponents(
            ...
            textDisplay => textDisplay.setContent(`Failed to fetch an image for the **${category}** category. The API seems shy about this one. Please try again later.`)
        )
        .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small));
    return { components: [container], flags: MessageFlags.IsComponentsV2, ephemeral: true };
};
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('sixtynine')
        .setDescription('Presents a playful and intimate encounter. ♋ (NSFW channels only)'),

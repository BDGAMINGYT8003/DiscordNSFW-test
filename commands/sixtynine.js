...
const createCooldownPayload = (timeLeft) => {
    const container = new ContainerBuilder()
        ...
        .addTextDisplayComponents(
            textDisplay => textDisplay.setContent('### ⏰ Patience, Playful Soul'),
            textDisplay => textDisplay.setContent(`You must wait **${timeLeft.toFixed(1)}s** before witnessing another intimate moment. Intimacy requires timing.`)
        )
        .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small));
    return { components: [container], flags: MessageFlags.IsComponentsV2, ephemeral: true };
};
...
const createApiErrorPayload = (category) => {
    const container = new ContainerBuilder()
        ...
        .addTextDisplayComponents(
            ...
            textDisplay => textDisplay.setContent(`Failed to fetch an image for the **${category}** category. The API seems shy about this one. Please try again later. The NSFWHub API we use to fetch images is very strictly rate-limited, so the most likely reason for the error is due to this limitation.`)
        )
        .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small));
    return { components: [container], flags: MessageFlags.IsComponentsV2, ephemeral: true };
};
...
const generateSixtyninePayload = async (interactionOrMessage) => {
    try {
        const imageUrl = await imagePreloader.getImage();

        const container = new ContainerBuilder()
            .setAccentColor(0x696969) // A playful gray/silver for 69
            .addTextDisplayComponents(
                textDisplay => textDisplay.setContent('### A Playful & Intimate Connection! ♋')
            )
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small))
            .addMediaGalleryComponents(
                mediaGallery => mediaGallery.addItems(
                    item => item.setURL(imageUrl).setDescription('A delightful moment of mutual appreciation.')
                )
            );
        ...
        const errorPayload = createApiErrorPayload("sixtynine");
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('sixtynine')
        .setDescription('Presents a playful and intimate encounter. ♋'),
    ...
    async slashExecute(interaction) {
        ...
        const payload = await generateSixtyninePayload(interaction);
    ...
    async prefixExecute(message, args) {
        ...
        const payload = await generateSixtyninePayload(message);
    ...
    async handleComponent(interaction, componentArgs) {
        ...
        if (componentType === 'button' && action === 'reload') {
            ...
            try {await interaction.deferUpdate();
                const newPayload = await generateSixtyninePayload(interaction);
                // Check if generateSixtyninePayload returned an error payload
            ...
            }  catch (error) {console.error('Error handling sixtynine reload button:', error);
                // General error handling for other issues, send ephemeral error
                const errorPayload = createApiErrorPayload("sixtynine"); // Use the updated error payload function
                 if (interaction.replied || interaction.deferred) {
